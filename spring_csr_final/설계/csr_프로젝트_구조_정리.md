...
| 로그인 정보 | `model.addAttribute("user", ...)` | `authFetch('/api/my/info')` |
...
│   │   ├── UserController.java       # 내 정보 API (/api/my/info)
...
매 HTTP 요청 → 쿠키/헤더에서 access_token 추출
  → null이면 비인증 상태로 통과
  → refresh 토큰이면 통과 (/api/refresh/reissue로 갈 것)
...
```
POST /api/refresh/reissue (쿠키에서 refresh_token 자동 전송)
...
authenticated() → /api/logout, /api/my/info, /api/rooms, /api/room/**,
                  POST/PUT/DELETE /api/community/**, /api/comments/**, /api/files/upload
...
HomeController (@Controller)
  ...
  → 브라우저에서 JS 실행 → fetch('/api/community') → JSON → DOM 렌더링
```
...
| `tryRefreshToken()` | 토큰 재발급 (`POST /api/refresh/reissue`) |
