...
| DTO | 역할 | 사용 위치 |
|-----|------|-----------|
| `SessionUserDTO` | 세션 저장용 사용자 정보 (id, username, nickname, email, roles) | CustomUserDetails 내부, 세션 |
...
> **Entity → DTO 변환 원칙 준수**:  
> CustomUserDetails는 필드를 직접 선언하지 않고 `SessionUserDTO`를 composition으로 갖는다.  
> `CustomUserDetails.from(entity)` → 내부에서 `SessionUserDTO.from(entity)` 재사용 + password 추가.  
...
|----------|----------|------|------|
| admin | test1234 | ADMIN,USER | 관리자 — /admin 접근 가능 |
| user1~15 | test1234 | USER | 일반 사용자 |
