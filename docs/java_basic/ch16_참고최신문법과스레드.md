...
description: Java 최신 문법(record, switch 표현식 등)과 멀티스레드 기초
---
...
![현대 Java 기능 지도]({{ '/java_basic/java_basic_images/ch16/modern-java-feature-map.svg' | relative_url }})
...
주의: switch 패턴 매칭은 Java 17에서는 preview 기능이라 기본 설정으로는 컴파일되지 않는다(`--enable-preview` 필요). Java 21에서 정식화됐다.  
이 강의(Java 17 기준)에서는 5.1의 instanceof 패턴까지만 사용한다.
...
![스레드 상태 생명주기]({{ '/java_basic/java_basic_images/ch16/thread-state-lifecycle.svg' | relative_url }})
...
경쟁 상태를 직접 눈으로 확인해 보자.

```java
static int count = 0;

public static void main(String[] args) throws InterruptedException {
    Runnable task = () -> { for (int i = 0; i < 10_000; i++) count++; };
    Thread t1 = new Thread(task);
    Thread t2 = new Thread(task);
    t1.start(); t2.start();
    t1.join(); t2.join();
    System.out.println(count); // 20000이 아닐 수 있다!
}
```

`count++`는 읽기 → +1 → 쓰기의 3단계 연산이라, 두 스레드가 중간에 서로 끼어들면 증가분이 사라진다.  
다음 절의 synchronized/원자 클래스가 바로 이 문제를 해결하는 도구다.

![두 스레드의 count++ 인터리빙]({{ '/java_basic/java_basic_images/ch16/race-condition-interleaving.svg' | relative_url }})
...
```java
// f.get()은 InterruptedException/ExecutionException(checked 예외)을 던지므로
// 이 코드를 담는 메소드에 throws Exception이 필요하다 (또는 try-catch로 처리)
...
queue.put("order-1");          // 가득 차 있으면 빈자리가 하나 생길 때까지 대기
String item = queue.take();    // 비어 있으면 들어올 때까지 대기
```

`put/take`가 알아서 대기해 주므로 생산자-소비자 구현이 간단해진다. (문제 F.1 참고)

---

## 8. 데드락/라이브락/기아

- 데드락(Deadlock): 두 스레드가 서로가 가진 락을 기다리며 영원히 멈추는 상태
- 라이브락(Livelock): 스레드들이 서로 양보만 반복하며 실제로는 아무도 진행하지 못하는 상태
- 기아(Starvation): 특정 스레드만 계속 자원(CPU/락)을 할당받지 못하는 상태
...
> 정답 예시: [ch16 문제 답안](문제답안/ch16_문제답안.md)
...
- Consumer: 스레드풀에서 처리
- 실패 시 재시도/로그
2. 최신 문법(record/switch 표현식)을 일부 적용해 코드 가독성을 개선하시오.
