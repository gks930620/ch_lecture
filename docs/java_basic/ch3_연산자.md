...
layout: default
title: ch3_연산자
description: 산술/비교/논리/비트/시프트/삼항 연산자와 평가 규칙
---
...
```java
double a = 0.1 + 0.2;
System.out.println(a == 0.3); // false (0.1 + 0.2는 정확히 0.30000000000000004)
```
...
> `new String`과 참조(객체를 가리키는 주소) 개념은 ch6(클래스와 객체)에서 자세히 다룬다.  
> 지금은 "문자열 내용 비교는 `==`가 아니라 `equals`"라는 결론만 기억하면 된다.
...
이 패턴이 NPE(NullPointerException, 아무 객체도 가리키지 않는 `null`에 접근할 때 나는 에러)를 줄이는 핵심이다.  
`null`과 객체 참조는 ch6에서 자세히 배우므로, 여기서는 "`&&`는 왼쪽이 false면 오른쪽을 아예 실행하지 않는다"는 동작만 이해하면 된다.
...
![단락 평가 흐름도]({{ '/java_basic/java_basic_images/ch3/short-circuit.svg' | relative_url }})
...
![시프트 연산 시각화]({{ '/java_basic/java_basic_images/ch3/shift-operators.svg' | relative_url }})
...
문제에서 "입력받아"는 `Scanner`를 사용한 콘솔 입력을 의미한다.
...
> 정답 예시: [ch3 문제 답안](문제답안/ch3_문제답안.md)

> 아직 배우지 않은 내용이 필요한 문제 안내
> - B1~B3, G1: 조건 분기(`if`/`switch`)가 필요합니다. 삼항 연산자로 도전해 보거나, ch4 학습 후 복습 문제로 풀어도 됩니다.
> - C2, D2, G2~G3: "함수/메서드 작성"은 메서드 학습(ch6) 전이므로, 지금은 `main` 안의 코드로 작성해도 됩니다.
> - F: 반복문(`for`)은 ch4에서 배웁니다. ch4 학습 후 풀어도 됩니다.
