...
layout: default
title: ch11_제네릭
description: 제네릭 타입, 와일드카드와 PECS, 타입 소거
---
...
> 예제에 나오는 `List`/`ArrayList`는 ch12(컬렉션)에서 자세히 배운다.  
> 여기서는 "값을 순서대로 담는 목록" 정도로만 이해해도 충분하다.

![제네릭 컴파일타임 vs 런타임]({{ '/java_basic/java_basic_images/ch11/generics-compile-runtime.svg' | relative_url }})
...
![extends는 읽기, super는 쓰기 — PECS]({{ '/java_basic/java_basic_images/ch11/pecs-wildcard.svg' | relative_url }})
...
`extends` 목록에 왜 추가(`add`)가 제한되는지 직접 확인해 보자:

```java
List<Integer> ints = new ArrayList<>(List.of(1, 2, 3));
List<? extends Number> nums = ints; // Integer 리스트를 "Number의 하위 무언가" 시각으로 참조
// nums.add(4);            // 컴파일 오류: 실제 원소 타입을 몰라 추가 불가
Number n = nums.get(0);    // 읽기는 안전 — n은 1
```

흔히 "extends는 읽기 전용"이라고 요약하지만, 정확히는 컴파일러가 실제 원소 타입을 알 수 없어  
잘못된 타입이 들어가는 것을 막는 것이다(예외적으로 `null`은 추가 가능).

> 실습 소스: `src/P5와일드카드타입파라미터/` — `Course.register1/2/3`이 각각 `<?>`, `<? extends Human>`, `<? super Worker>`를 시연한다.
...
// 컴파일 후 (바이트코드 관점 — 타입 정보 제거)
List names = new ArrayList();
```

![제네릭 컴파일 vs 런타임]({{ '/java_basic/java_basic_images/ch11/generics-compile-runtime.svg' | relative_url }})
...
```java
// new T() 대신 Class<T>를 받아 런타임에 타입 정보를 활용
...
primitive stream(ch14에서 배울 `IntStream`) 등 대안을 검토한다.
...
> 정답 예시: [ch11 문제 답안](문제답안/ch11_문제답안.md)

> 실습 소스 `src/quest`의 Q1~Q4는 일부 문항의 예시 답안입니다 — Q1: A1·A2(Box), Q2: B1(swap), Q3: C1(경계 타입 평균), Q4: D1·D2 응용(extends/super 복사).  
> 나머지 문항(B2, C2, D3, E, F)은 예시 답안 없이 직접 풀어보세요.
