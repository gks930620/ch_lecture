...
| 구분 | HTTP | WebSocket |
|------|------|-----------|
| 통신 방향 | 요청-응답 (클라이언트가 먼저 요청해야만 서버가 응답) | 양방향 (Full-Duplex) |
...
Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==   ← 핸드셰이크 검증용 랜덤 키 (암호화·인증용 아님)
...
- `Sec-WebSocket-Key` / `Sec-WebSocket-Accept` 교환은 **핸드셰이크 검증용**이다.
  - 서버가 WebSocket 프로토콜을 이해하는지(정상 응답을 만들 수 있는지) 확인하고,
  - 캐싱 프록시 등이 이 응답을 일반 HTTP 응답으로 오인·재사용하지 못하게 막는 용도다.
  - ⚠️ **암호화·인증·CSRF 방어 목적이 아니다** (RFC 6455: "이 값은 서버 인증이나 보안 제공에 사용되지 않는다").
    WebSocket에서 CSRF와 관련된 것은 `Sec-WebSocket-Key`가 아니라 **`Origin` 헤더 검증**이다.
    (참고: 이 프로젝트는 학습용이라 `SecurityConfig`에서 CSRF 자체를 `disable()` 한다.)
...
```
SockJS 연결 시도 순서 (대표적인 경우):
1. WebSocket       ← 최우선 (ws://)
2. XHR Streaming   ← WebSocket 안 되면 HTTP 스트리밍
3. XHR Polling     ← 그것도 안 되면 HTTP 폴링
```
> 실제 SockJS의 전송 후보는 이 3가지 외에도 더 많다(eventsource, iframe 계열 등).
> 여기서는 이해를 돕기 위해 대표적인 3단계로 단순화했다.
...
채팅방 페이지 자체가 로그인 필수이므로, **정상적인 화면 흐름에서는** 로그인한 사용자의 실제 닉네임이 `sender`로 사용된다.

> ⚠️ **다만 서버가 `sender`의 진위를 보장하지는 않는다.**
> `ChatMessageController`는 클라이언트가 STOMP 메시지 body에 담아 보낸 `sender`를 그대로 신뢰해 브로드캐스트한다.
> WebSocket 레벨 인증이 없으므로, 악의적인 클라이언트는 `sender`를 남의 닉네임 등 임의 값으로 위조해 보낼 수 있다.
> 이 절 앞부분의 "운영 환경에서 WebSocket 보안이 필요하다면" 박스처럼 CONNECT 시 인증 검증을 추가해야 진짜로 보장된다(학습용 프로젝트의 한계).
