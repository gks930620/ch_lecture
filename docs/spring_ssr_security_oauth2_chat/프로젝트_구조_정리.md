...
> 📌 **읽기 전 안내**
> 아래 "아키텍처 원칙 / API 설계 규칙 / JPA 설계"의 일부(게시판·댓글·파일·글 상세 조회 전략 등)는
> 여러 프로젝트에 공통으로 적용하는 **일반 보일러플레이트 규칙**이다.
> **이 프로젝트에 실제로 구현된 기능은 로그인(Security/OAuth2)과 실시간 채팅뿐**이며, 게시판·댓글·파일 기능은 없다.
> 따라서 댓글/파일 관련 코드를 찾으려 하지 말고, 해당 규칙은 "이럴 땐 이렇게 한다"는 참고용으로 읽으면 된다.
...
| DTO | 역할 | 사용 위치 |
|-----|------|-----------|
| `SessionUserDTO` | 세션 저장용 사용자 정보 (id, username, nickname, email, roles, provider) | CustomUserAccount 내부, 세션 |
...
- `provider` 필드 추가 (OAuth2 provider명: "kakao"/"google", 폼 로그인 시 null) — 상세는 [`OAuth2_로그인_설명.md`](OAuth2_로그인_설명.md) 참고
- `updatePassword()` 메서드 추가 (BCrypt 인코딩용)
- password 컬럼: **폼 로그인 사용자**는 BCrypt 해시값 저장(60자), **OAuth2 사용자**는 로그인 검증에 쓰이지 않는 더미값(`{noop}oauth2user`) 저장
- (참고) `is_deleted` 필드 + `softDelete()` + 클래스 레벨 `@SQLRestriction("is_deleted = false")`로 소프트 삭제를 지원 — 조회 시 삭제된 사용자는 자동 제외됨. (현재 회원 삭제 화면은 없음, 향후 확장용)
