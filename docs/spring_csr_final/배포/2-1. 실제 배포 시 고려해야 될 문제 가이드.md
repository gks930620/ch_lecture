...
### 3. CORS 설정 (CorsConfig.java) - ✅ Bean 방식 적용됨 / ⬜ 운영 도메인은 직접 추가 필요
> 📌 CORS를 `CorsConfigurationSource` **Bean 방식으로 등록한 것은 적용 완료**입니다.
> 다만 실제 `CorsConfig.java`의 허용 Origin은 `localhost:3000/5173/8080`뿐이고 **운영 도메인은 주석 예시로만** 들어 있습니다.
> `allowCredentials(true)` 상태라 운영 배포 시 본인 도메인을 허용 목록에 직접 추가하지 않으면 "Invalid CORS request"가 다시 날 수 있습니다.
...
#### 해결 방법 (Bean 방식 적용됨 ✅)
`CorsConfigurationSource` Bean으로 변경. **실제 `CorsConfig.java`의 현재 모습**은 다음과 같습니다:
```java
@Bean
public CorsConfigurationSource corsConfigurationSource() {
    ...
    configuration.setAllowedOrigins(Arrays.asList(
        ...
        // ⬇️ 운영 배포 시 본인 도메인을 여기에 직접 추가해야 함 (현재는 주석 예시)
...
> `JwtLoginFilter`는 Spring Bean이 아니므로 SecurityConfig에서 생성자 파라미터로 `secureCookie` 전달
> `CustomLogoutSuccessHandler`는 `@Component`이므로 `@Value` 직접 사용
...
> 📌 아래 "현재 설정"은 **로컬용 `application.yml`** 기준입니다.
> **운영용 `application-prod.yml`은 이미** `include: health, info` + `show-details: when_authorized`로 되어 있어 metrics는 노출되지 않습니다.
> (권장값 `show-details: never`와는 다르니, 더 엄격히 하려면 아래 권장 설정 참고)

#### 현재 설정 (로컬 application.yml)
```yaml
management:
  endpoints:
    web:
