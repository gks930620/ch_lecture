웹/앱 OAuth2 + JWT 로그인 방식
- 웹 OAuth2 시작 URL은 `Oauth2LoginController`의 `/custom-oauth2/login/web/{provider}`를 사용한다.
  (`/oauth2/authorization/*` 기본 URL 대신 커스텀 URL을 사용해 흐름을 명확히 제어)
- 웹 OAuth2 성공 후 `OAuth2LoginSuccessHandler`에서 쿠키를 발급하고 `/`로 리다이렉트한다.
- 앱은 WebView OAuth2 시작 URL을 타지 않고, 네이티브 SDK 로그인 후 서버 API로 JWT를 발급받는다.
  (`/api/oauth2/providers/{provider}/tokens`)
- 이후 인증 방식은 OAuth2/일반 로그인 구분 없이 동일하다.
  (클라이언트가 보낸 토큰을 서버가 검증)
...
### 앱 OAuth2 (Native SDK 방식)
- 앱은 카카오/구글 모두 네이티브 SDK 방식으로 로그인한다.
- 네이티브 로그인 후 사용자 정보를 서버
  `/api/oauth2/providers/{provider}/tokens`로 전송한다.
- 서버의 `AppOAuth2Controller`가 사용자 생성/조회 후 JWT 토큰을 발급한다.
...
- 앱에서 네이티브 로그인 후 사용자 정보를 서버
  `/api/oauth2/providers/{provider}/tokens`로 전송
