...
> ⚠️ 401/토큰 재발급을 유발하는 건 **페이지 접속 자체가 아니라, 그 페이지가 호출하는 인증 필요 API**입니다.
> `/community/**`, `/mypage` 같은 HTML 페이지는 `permitAll`(공개)이라 접속만으로는 401이 나지 않습니다.
- 예: `/community/write` 페이지에서 **게시글 작성 → `POST /api/communities` 호출**
- 또는 `/mypage` 페이지가 로드되며 **`GET /api/users/me` 호출**
#### Step 4: 콘솔 로그 확인
성공 시 다음 로그가 표시됨:
```
