...
> ℹ️ **이 문서는 이 프로젝트(spring_csr_jwt_access_refresh) 전용 설명이 아니라, 개인 보일러플레이트 설계 메모입니다.**  
> OAuth2(카카오/구글)·파일 업로드·게시판·Docker·Swagger 등 이 프로젝트에는 없는 기능까지 폭넓게 다룹니다.  
> 또 여기 적힌 refresh 토큰 값(4시간·쿠키 저장)은 일반 예시이며, **이 프로젝트는 refresh 토큰을 DB에 저장**합니다.  
> 이 프로젝트의 실제 스펙은 [`jwt 토큰 설명.md`](<jwt 토큰 설명.md>)를 기준으로 보세요.
---
## 📐 아키텍처 원칙
### Layer Architecture
```
