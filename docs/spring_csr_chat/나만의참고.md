...
> ⚠️ **먼저 읽어주세요 — 이 문서는 `spring_csr_chat` 프로젝트 설명이 아닙니다.**
>
> 이 글은 저자가 여러 프로젝트에서 공통으로 쓰는 **범용 Spring Boot 보일러플레이트 설계 메모**입니다.
> 아래에 나오는 OAuth2(카카오·구글), 파일 업로드(Strategy/Supabase), 게시판·댓글(`community/`),
> `ApiResponse`/`PageResponse`/`GlobalExceptionHandler`, `logback-spring.xml`, Docker, Actuator,
> `application-prod.yml`, QueryDSL 등은 **이 채팅 프로젝트 소스에는 존재하지 않습니다.**
> 특히 아래 "보일러플레이트 구현 현황" 표의 ✅ 표시는 이 프로젝트가 아니라 **범용 보일러플레이트 기준**입니다.
> JWT 만료값(30분/4시간, 쿠키 저장) 설명도 이 프로젝트 실제 설정(`application.yml`의 테스트용 5분/10분, localStorage 저장)과 다릅니다.
>
> **채팅 프로젝트의 실제 구조·동작은 [`stomp설명.md`](stomp설명.md)를 참고하세요.**
...
| **웹소켓** | 실시간 양방향 통신 (채팅, 알림) | CSR + JWT 완성 후 (STOMP + SockJS + JWT 인증, SSR 단계에서는 하지 말 것) |
