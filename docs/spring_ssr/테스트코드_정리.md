...
| 검증 대상 | 설명 |
|-----------|------|
| **View Name** | 정확히 `community/list` (뷰 이름 — 확장자 없음)를 반환하는가? |
| **Model Attributes** | 화면에 그려줄 데이터가 Model에 제대로 담겼는가? ← **가장 중요** |
| **Redirection** | 로그인 성공 후 메인 페이지로 리다이렉트 되는가? |

```java
// SSR 방식의 전형적인 MockMvc 테스트
// ※ 이 프로젝트에 실제로 있는 테스트는 아니고, 작성한다면 이런 형태라는 예시
//   (현재 테스트는 DemoApplicationTests의 contextLoads() 1개뿐)
...
// ※ 댓글(comments)은 이 프로젝트에서는 Model에 담지 않는다 —
//   detail.html의 JS가 댓글 API를 fetch로 호출해서 그리는 구조이기 때문
```
...
- CSR에서는 API별 응답이 잘 오면 전체 로직이 잘 돌아간다고 확신할 수 있어 통합 테스트의 가치가 크다.
  반면 SSR은 화면(HTML)까지 검증하는 테스트의 가성비가 낮으므로,
  Service/Repository 단위 테스트 + Controller는 뷰 이름/Model/리다이렉션만 가볍게 확인하는 전략이 합리적이다.
