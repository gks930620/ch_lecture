...
> **이 프로젝트의 실제 사례**: `CommunityController.delete()`가 바로 이 구조다 —
> `fileService.detachFilesByRefId()` → `commentService.deleteCommentsByCommunityId()` → `communityService.deleteCommunity()`를
> 트랜잭션 없이 순차 호출한다. 중간에 실패하면 앞의 작업은 롤백되지 않는다.  
> 그래서 최소한 **권한 체크(`validateOwner`)를 맨 앞에** 두어 권한 없는 요청이 파일/댓글을
> 먼저 지워버리는 일은 막아두었다. 제대로 개선하려면 아래처럼 Facade에서 `@Transactional`로 묶으면 된다.
