...
files 테이블
├── id
├── ref_type    (도메인 구분 — 이 프로젝트: COMMUNITY, USER)
├── ref_id      (각 도메인의 PK)
├── file_usage  (같은 도메인 안에서의 용도 구분 — THUMBNAIL / IMAGES / ATTACHMENT)
├── original_file_name
├── stored_file_name
├── file_path
├── file_size
...
> ※ 이 프로젝트에는 별도의 FileUtil 클래스는 없고, `FileService`가 이 역할(UUID 파일명 생성,
> `Files.write`로 물리 저장)과 DB 저장(files 테이블 INSERT)을 함께 수행한다.
