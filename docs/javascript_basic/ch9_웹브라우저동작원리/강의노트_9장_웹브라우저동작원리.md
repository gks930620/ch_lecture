...
│     네트워킹                            │  ← HTTP 요청 처리
├────────────────────────────────────────┤
│     JavaScript 엔진 (V8 등)             │  ← JavaScript 실행(JIT 컴파일 포함)
├────────────────────────────────────────┤
│     UI 백엔드                           │  ← 기본 위젯 그리기
├────────────────────────────────────────┤
...
![브라우저 렌더링 파이프라인]({{ '/javascript_basic/javascript_basic_images/ch9/rendering-pipeline.svg' | relative_url }})
...
\* 별도 합성(compositing) 레이어로 승격된 경우에만 리플로우·리페인트 없이 GPU에서 처리됩니다. 레이어로 분리되지 않으면 opacity 등은 상황에 따라 리페인트가 발생할 수 있습니다.
...
for (let i = 0; i < 100; i++) {
    const div = document.createElement('div');
    document.body.appendChild(div);  // 라이브 DOM을 100번 조작 → 레이아웃을 계속 무효화
}
...
for (let i = 0; i < 100; i++) {
    const div = document.createElement('div');
    fragment.appendChild(div);  // 메모리에서만 작업 (레이아웃 영향 없음)
}
document.body.appendChild(fragment);  // 라이브 DOM 조작은 1번뿐
// 참고: 브라우저는 레이아웃을 즉시 계산하지 않고 배치(지연)합니다.
//       중간에 offsetWidth 등을 읽어 강제하지 않는 한 100번의 동기 리플로우가 나는 건 아니지만,
//       라이브 DOM을 자주 건드릴수록 무효화·재계산 비용이 커집니다.
```
...
function updateList(items) {
    ...
    items.forEach(item => {
        ...
        li.textContent = item;
        list.appendChild(li);  // 라이브 DOM을 매번 조작 → 레이아웃 반복 무효화
    });
}
...
function updateListOptimized(items) {
    ...
    list.innerHTML = '';  // 라이브 DOM 조작 1회
    list.appendChild(fragment);  // 라이브 DOM 조작 1회
    // 라이브 DOM 조작을 2회로 줄여 무효화·재계산을 최소화!
}
```
