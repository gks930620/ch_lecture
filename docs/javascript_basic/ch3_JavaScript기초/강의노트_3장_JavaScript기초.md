...
> 🧩 **실습 미리보기(part04)**: 실습 파일에는 **고차 함수(함수를 반환하는 함수)**와 **IIFE(즉시 실행 함수)**도
> 개념 소개 수준으로 등장합니다. 형태만 익히면 되고, "왜 값을 기억하는지"의 원리인 **클로저는 4장**에서 자세히 배웁니다.
...
> 🧩 **실습 미리보기(part05)**: 실습 파일에는 **구조 분해 할당**(`const { name, age } = user;`)도 나옵니다.
> 객체에서 원하는 속성을 같은 이름의 변수로 한 번에 꺼내는 새 문법으로, 지금 배운 객체 지식만으로 이해할 수 있습니다.
