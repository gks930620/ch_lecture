...
- 기본값: `${empty param.page ? 1 : param.page}` (삼항 연산자 `? :`는 **EL 2.0(JSP 2.0)부터** 지원). 참고로 `ne null`은 파라미터가 아예 없을 때만 걸러지고 빈 문자열 `""`은 통과하므로, 빈 문자열까지 함께 처리하려면 `empty`를 쓰는 편이 안전하다.
...
- 가능하면 로직은 서블릿/비즈니스 레이어로 두고 JSP는 표시만 하도록 설계. (참고: 실습 파일 `08_jstl_demo.jsp`는 편의상 스크립틀릿 `<% ... request.setAttribute(...) %>`로 데이터를 만들지만, **실무에서는 서블릿에서 속성을 바인딩**하고 JSP는 EL/JSTL로 출력만 하는 것이 원칙.)
